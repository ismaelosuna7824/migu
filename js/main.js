...
function creaNuevaDB(tx){
	...
	tx.executeSql("INSERT INTO agenda_curso (id,nombre,domicilio,telefono,categoria,foto,nota, email) VALUES (1,'Mónica','Olivarría','+6699900970','heladeria','','notas','m.olivarria@ccumazatlan.mx')");
}
...
function initForm(){
	...
	$("#cat_polleria").trigger("click").trigger("click")
}
...
function queryDBUpdateForm(tx){
	...
	tx.executeSql('UPDATE agenda_curso SET nombre="'+$("#txt_nombre").val()+'", domicilio="'+$("#txt_domicilio").val()+'",telefono="'+$("#txt_telefono").val()+'",email="'+$("#txt_mail").val()+'",nota="'+$("#txt_nota").val()+'",categoria="'+cat+'",foto = "'+$.imageURL+'" WHERE id='+$.id);
}
...
function queryDBInsertForm(tx){
	...
	tx.executeSql("INSERT INTO agenda_curso (nombre,domicilio,telefono,categoria,foto,nota,email) VALUES ('"+$("#txt_nombre").val()+"','"+$("#txt_domicilio").val()+"','"+$("#txt_telefono").val()+"','"+cat+"','"+$.imageURL+"','"+$("#txt_nota").val()+"','"+$("#txt_mail").val()+"')", [], newFormSuccess, errorDB);
}
...
function eliminar(){
	if(db != null){
		db.transaction(queryEliminar, errorDB, EliminarSuccess);
	}
}

function queryEliminar(tx){
	var cat = $("#cajaCategorias").find("input:checked").val();
	tx.executeSql('delete from agenda_curso WHERE id='+$.id);
}
function EliminarSuccess(tx) {
